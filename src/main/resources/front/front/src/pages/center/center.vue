    ...
    <el-tabs tab-position="left" :style='{"background":"none"}' @tab-click="handleClick">
      <el-tab-pane label="个人中心">
        <el-form class="center-preview-pv" ref="sessionForm" :model="sessionForm" :rules="rules" label-width="110px">
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='zhiyuanzhe'" label="志愿者账号" prop="zhiyuanzhezhanghao">
            <el-input v-model="sessionForm.zhiyuanzhezhanghao" placeholder="志愿者账号" readonly></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='zhiyuanzhe'" label="密码" prop="mima">
            <el-input type="password" v-model="sessionForm.mima" placeholder="密码"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='zhiyuanzhe'" label="志愿者姓名" prop="zhiyuanzhexingming">
            <el-input v-model="sessionForm.zhiyuanzhexingming" placeholder="志愿者姓名" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='zhiyuanzhe'" label="照片">
            <el-upload
              class="avatar-uploader"
              :action="uploadUrl"
              :headers="headers"
              :show-file-list="false"
              :on-success="zhiyuanzhezhaopianHandleAvatarSuccess">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='zhiyuanzhe'" label="性别">
            <el-select v-model="sessionForm.xingbie" placeholder="请选择性别" >
              <el-option v-for="(item, index) in dynamicProp.xingbie" :key="index" :label="item" :value="item"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='zhiyuanzhe'" label="年龄" prop="nianling">
            <el-input v-model="sessionForm.nianling" placeholder="年龄" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='zhiyuanzhe'" label="手机号码" prop="shoujihaoma">
            <el-input v-model="sessionForm.shoujihaoma" placeholder="手机号码" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='zhiyuanzuzhi'" label="组织编号" prop="zuzhibianhao">
            <el-input v-model="sessionForm.zuzhibianhao" placeholder="组织编号" readonly></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='zhiyuanzuzhi'" label="密码" prop="mima">
            <el-input type="password" v-model="sessionForm.mima" placeholder="密码"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='zhiyuanzuzhi'" label="组织名称" prop="zuzhimingcheng">
            <el-input v-model="sessionForm.zuzhimingcheng" placeholder="组织名称" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='zhiyuanzuzhi'" label="组织logo">
            <el-upload
              class="avatar-uploader"
              :action="uploadUrl"
              :headers="headers"
              :show-file-list="false"
              :on-success="zhiyuanzuzhitupianHandleAvatarSuccess">
              <img :style='{"border":"1px solid #ddd","cursor":"pointer","color":"#333","borderRadius":"6px","textAlign":"center","background":"linear-gradient(240deg, rgba(246,246,246,1) 0%, rgba(246,246,246,1) 50%, rgba(238,238,238,1) 50%, rgba(238,238,238,1) 100%)","width":"100px","fontSize":"32px","lineHeight":"100px","height":"100px"}' v-if="sessionForm.tupian" :src="baseUrl + sessionForm.tupian" class="avatar">
              <i v-else class="el-icon-plus avatar-uploader-icon"></i>
            </el-upload>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='zhiyuanzuzhi'" label="成立时间">
            <el-date-picker
              v-model="sessionForm.chenglishijian"
              type="date"
              placeholder="成立时间" >
            </el-date-picker>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='zhiyuanzuzhi'" label="负责人" prop="fuzeren">
            <el-input v-model="sessionForm.fuzeren" placeholder="负责人" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='zhiyuanzuzhi'" label="联系电话" prop="lianxidianhua">
            <el-input v-model="sessionForm.lianxidianhua" placeholder="联系电话" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='zhiyuanzuzhi'" label="组织简介" prop="zuzhijianjie">
            <el-input v-model="sessionForm.zuzhijianjie" placeholder="组织简介" ></el-input>
          </el-form-item>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      if ('zhiyuanzuzhi' == this.userTableName) {
        this.$set(this.sessionForm, 'zuzhibianhao', null);
      }
      if ('zhiyuanzuzhi' == this.userTableName) {
        this.$set(this.sessionForm, 'mima', null);
      }
      if ('zhiyuanzuzhi' == this.userTableName) {
        this.$set(this.sessionForm, 'zuzhimingcheng', null);
      }
      if ('zhiyuanzuzhi' == this.userTableName) {
        this.$set(this.sessionForm, 'tupian', null);
      }
      if ('zhiyuanzuzhi' == this.userTableName) {
        this.$set(this.sessionForm, 'chenglishijian', null);
      }
      if ('zhiyuanzuzhi' == this.userTableName) {
      ...
      if ('zhiyuanzuzhi' == this.userTableName) {
        this.$set(this.sessionForm, 'zuzhijianjie', null);
      }
      if ('zhiyuanzuzhi' == this.userTableName) {
      ...
      if ('zhiyuanzuzhi' == this.userTableName) {
        this.$set(this.rules, 'zuzhibianhao', [{ required: true, message: '请输入组织编号', trigger: 'blur' }]);
      }
      if ('zhiyuanzuzhi' == this.userTableName) {
        this.$set(this.rules, 'mima', [{ required: true, message: '请输入密码', trigger: 'blur' }]);
      }
      if ('zhiyuanzuzhi' == this.userTableName) {
        this.$set(this.rules, 'zuzhimingcheng', [{ required: true, message: '请输入组织名称', trigger: 'blur' }]);
      }
			if ('zhiyuanzuzhi' == this.userTableName) {
    ...
    methods: {
      ...
      zhiyuanzhezhaopianHandleAvatarSuccess(res, file) {
        if (res.code == 0) {
          if ('zhiyuanzhe' == this.userTableName) {
      ...
      zhiyuanzuzhitupianHandleAvatarSuccess(res, file) {
        if (res.code == 0) {
          if ('zhiyuanzuzhi' == this.userTableName) {
